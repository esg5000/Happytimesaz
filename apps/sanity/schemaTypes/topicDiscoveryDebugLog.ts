 ...
 * (the pipeline's topicDiscovery.ts writeShadowLog, still written as-is) with
 * something durable and queryable regardless of which host ran the job.
 */
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'array',
      description:
        'The full near-deduped, pre-cap Stage 0 pool (before the pipeline\'s STAGE1_CANDIDATE_CAP recency cut). Does NOT include the raw pre-dedupe pool (~1000-1500 items); only counts of that are recorded on this run\'s syncRun.topicDiscoveryUsage.',
      of: [
        {
          type: 'object',
          fields: [
            ...
            defineField({
              ...
              type: 'boolean',
              description:
                'OBSERVE-ONLY (the pipeline\'s topicDiscovery.ts flagPossibleNearDupes) — shares a distinguishing title phrase + publish-time proximity with another pool item but fell under the Jaccard merge threshold, so it was NOT actually merged. For manual review only.',
            }),
            defineField({
    ...
    defineField({
      ...
      type: 'array',
      description:
        'Kept Stage 0-2 candidates skipped at persist time because their sourceUrl matched something already seen (an existing topicCandidate doc of any status, or a published post).',
      of: [
        {
          type: 'object',
          fields: [
